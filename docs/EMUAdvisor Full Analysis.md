...
- **Localization:**  Verify that language detection/routing works: English queries search English corpus by default.  If a Turkish query is asked, ensure it searches Turkish corpus first.  If cross-corpus is selected, verify both languages searched with clear labeling.  Check that text snippets remain untranslated (no silent mixing).  *Acceptance:* Answers should cite in the query language unless overridden.  The UI indicates source language.  (The system spec forbids mixing languages silently.)  
